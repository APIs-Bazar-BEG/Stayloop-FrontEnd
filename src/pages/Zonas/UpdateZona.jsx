  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/zonas", {
          state: {
            successMessage: `Zona "${zonaData.nombre}" actualizada correctamente.`,
          },
        });
      }, 1500);
    } catch (err) {
      ...
      const errorMessage =
        ...
        "Error al actualizar la zona. Verifica los datos.";
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="text-center">
          ...
          <p className="mt-4 text-lg leading-6 text-gray-500">
            Cambia los datos de la zona ID:{" "}
        ...
        <form
          ...
          className="mt-8 space-y-6 bg-white p-8 shadow-xl rounded-2xl"
        >
          {/* Mensajes de feedback */}
          {error && (
            <div className="p-3 mb-4 text-sm text-red-700 bg-red-100 rounded-xl border border-red-200">
              {error}
            </div>
          )}
          {success && (
            <div className="p-3 mb-4 text-sm text-green-700 bg-green-100 rounded-xl border border-green-200">
              ¡Actualización exitosa! Redirigiendo...
            </div>
          )}
              ...
              {hoteles.length > 0 ? (
                hoteles.map((hotel, index) => (
                  <li key={index} className="text-sm">
                    <span className="font-semibold">{hotel.nombre}</span> (ID:{" "}
                    {hotel.id})
                  </li>
                ))
              ) : (
