...
const DeleteZona = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            navigate('/zonas', { state: { successMessage: 'Zona eliminada correctamente.' } });
        } catch (err) {
            setDeleting(false);
            // Asumiendo que el error puede ser una restricción de llave foránea (hoteles asociados)
            const errorMessage = err?.response?.data?.message || 'Hubo un error al intentar eliminar la zona. Podría tener hoteles asociados.';
            setError(errorMessage);
        }
    };

    if (loading) {
        return <div className="text-center py-10 text-gray-500">Cargando detalles de la zona...</div>;
    }
    ...
    if (!zona) {
        return <div className="text-center py-10 text-red-500 font-bold">No se encontró la zona para eliminar.</div>;
    }
        ...
        <div className="flex flex-1 justify-center py-10 px-4 sm:px-6 lg:px-8">
            <div className="w-full max-w-lg">
                ...
                <div className="text-center">
                    <h2 className="text-3xl font-bold tracking-tight text-red-700 sm:text-4xl">Confirmar Eliminación</h2>
                    <p className="mt-4 text-lg leading-6 text-gray-700">
                        Estás a punto de eliminar la siguiente zona. Esta acción es irreversible.
                    </p>
                </div>
                ...
                <div className="mt-8 space-y-6 bg-white p-8 shadow-lg sm:rounded-2xl border-2 border-red-300">
                    ...
                    <p className="text-gray-600">
                        ID: <span className="font-mono bg-gray-100 p-1 rounded text-sm">{id}</span>
                    </p>
                    <p className="text-gray-600">
                    ...
                    {error && <div className="p-3 my-4 text-sm text-red-700 bg-red-100 rounded-lg">{error}</div>}
                    ...
                    <div className="flex flex-col sm:flex-row-reverse gap-4 pt-4">
                        <button 
                            ...
                            className={`flex w-full justify-center rounded-full border border-transparent
                                        ${deleting ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'} py-3 px-4 text-sm font-bold text-white
