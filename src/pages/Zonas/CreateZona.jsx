...
import { createZona, updateZona, getZonaById } from '../../services/zonasService'; 
...
const CreateZona = () => {
        ...
        if (isEditMode) {
            const fetchZonaData = async () => {
                try {
                    ...
                    const errorMessage = err.response?.data?.message || err.message || 'Fallo al cargar la zona para editar.';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const errorMessage = (err.response?.data?.message || err.toString() || "Error desconocido al guardar la zona.").trim();
            setError(errorMessage);
    ...
    if (error && isEditMode && !loading) {
        return <div className="text-center py-10 text-red-600 font-bold">Error de carga: {error}</div>;
    }
    ...
    return (
        <div className="flex flex-1 justify-center py-10 px-4 sm:px-6 lg:px-8 bg-[var(--background-color)]">
            <div className="w-full max-w-lg">
                ...
                {/* Formulario */}
                <form onSubmit={handleSubmit} className="mt-8 space-y-6 bg-white p-8 shadow-sm sm:rounded-2xl">

                    {/* Nombre */}
                    <div>
                        <label className="block text-sm font-medium text-[var(--text-primary)]" htmlFor="nombre">Nombre</label>
                        <div className="mt-1">
                            <input 
                                id="nombre"
                                name="nombre"
                        ...
                        {error && error.includes('obligatorio') && error.includes('nombre') && <span className="text-red-500 text-sm mt-1 block">El nombre es obligatorio.</span>}
                    </div>

                    {/* Descripción */}
                    <div>
                        <label className="block text-sm font-medium text-[var(--text-primary)]" htmlFor="descripcion">Descripción</label>
                        <div className="mt-1">
                            <textarea 
                                id="descripcion"
                                name="descripcion"
                    ...
                    <div className="flex flex-col sm:flex-row-reverse gap-4 pt-4">
                        ...
                        >
                            {loading ? 'Guardando...' : (isEditMode ? 'Guardar Cambios' : 'Crear Zona')}
                        </button>
