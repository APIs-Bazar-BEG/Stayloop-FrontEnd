...
const ReadZona = () => {
    ...
    if (error || !zona) {
        return <div className="text-center py-10 text-red-500 font-bold">{error || 'No se encontró la zona.'}</div>;
    }
        ...
        <div className="flex flex-1 justify-center py-10 px-4 sm:px-6 lg:px-8">
            <div className="w-full max-w-lg">
                ...
                <div className="text-center">
                    <h2 className={`text-3xl font-extrabold tracking-tight text-indigo-800 sm:text-4xl`}>
                        Detalle de Zona
                    </h2>
                    <p className={`mt-2 text-md leading-6 text-gray-500`}>
                        Información completa de la zona <span className="font-semibold">{zona.nombre}</span>
                    </p>
                </div>
                ...
                <div className="mt-8 space-y-6 bg-white p-8 shadow-xl rounded-2xl border border-gray-200">
                    ...
                    {/* ID */}
                    <div>
                        <label className={`block text-sm font-medium text-gray-900`} htmlFor="id">ID de Zona</label>
                        <div className="mt-1">
                            <input
                                id="id"
                                type="text"
                    ...
                    <div className="pt-4 border-t border-gray-100">
                        <label className={`block text-md font-bold text-indigo-700 mb-2`}>Hoteles Asociados</label>
                        <ul className={`list-disc list-inside text-gray-700 space-y-2 bg-indigo-50 p-4 rounded-xl`}>
                            {zona.hoteles && zona.hoteles.length > 0 ? (
                                zona.hoteles.map((hotel) => (
                                    // Asumiendo que hay una ruta para el detalle de hoteles
                                    <li key={hotel.id} className="text-sm hover:text-indigo-900 transition-colors">
                                        <Link to={`/hoteles/detail/${hotel.id}`} className="font-semibold underline">
